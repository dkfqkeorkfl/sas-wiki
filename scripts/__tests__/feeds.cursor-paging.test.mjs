...
//   ★ CX9(「`--ignore` 를 받고도 무시」)의 red 집합은 **C6 실측**으로 sas-wiki **IW1 · IW2 · IW6 ·
//     IW7** + webfront **IW3 · IW4** 이고, **IW5(텍스트 겹)만 green** 이다. 행동 겹이 조회 층에도
//     있어야 CX9 가 배선(IW5·텍스트 겹)과 구분된다.
//   ★ 같은 변이에서 **WA7·WA8 도 함께 red** 다 — 그 둘의 Arrange(억제 밀집 구간)가 억제 없이는
//     0건 페이지를 못 만들어 앵커가 무너지기 때문이지 커서 계약이 깨져서가 아니다. 회귀로 읽지 마라.
// ────────────────────────────────────────────────────────────────────────────

...
/** 🔴 v3 P1 · plan Task 7 · D5 — 리포트 생성 주체가 생성기에서 `validate.mjs --out <dir>` 로 이사한다. */
...
describe('배관 — 산출물 경로 소유권 (PL5 · `--out` 계약)', () => {
  it('PL5: `--out` 로 지정한 tmp **밖에는** 아무것도 쓰지 않는다', { timeout: 120_000 }, () => {
    // ★ `logs`·`cache` 가 명시된 경로가 아니라 스크립트 리포 기준이면, 병렬 테스트가 실 리포의
    //   **고정명 파일 하나**를 두고 경합한다 — 서로의 산출물을 덮어써 무작위로 깨진다.
    //
    // v3 P1(§4.10 「재작성」 · PL5): Task 7 이후 기본 summary 실행은 stdout payload 만 내고
    //   파일을 쓰지 않으므로, 「실 리포가 안 더러워진다」만 보면 자동 참(공허)이 된다. 그러나
    //   **「산출물 경로 소유권」은 지정한 출력 경로만 쓰는가라는
    //   관측면으로 살아 있다** — 그래서 경로를 **명시적으로 tmp 안에** 준 실행으로 축을 옮긴다.
    //
    ...
    //     ⓐ 다른 곳으로 샜다 → 지정한 `--out` 자리에 산출물이 실재하는지 단언
    ...
    const validateRun = run(VALIDATE, ['--out', reportDir])
    ...
    // 리포트 2형식은 `--out <dir>` 아래에만 생긴다(D5 · 파일명은 스크립트가 정한다).

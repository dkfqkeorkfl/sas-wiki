...
//
// RED 사유:
//   PL8 — `summary.schema.json` 은 지금 strict + required **6키**이고 `env` 가 없다.
//
// 왜 스키마를 따로 무는가: 이 스키마는 `additionalProperties: false` 다. 즉 생성기가 페이로드에
//   `env` 를 실으면서 스키마를 안 넓히면 **산출물이 자기 검증에 걸려 죽는다**(넓히면서
//   required 를 빠뜨리면 반대로 헤더 없는 파일이 통과한다). 두 방향 모두 `.mjs` 라 타입 체커가
//   못 잡고 런타임까지 드러나지 않는다. AR4(발행 파일 7키)·AR8 과 3자로 맞댄다.
//
...
describe('P4 배관 — 발행 아티팩트 스키마', () => {
  ...
  // **선언이 아니라 거동을 문다.** 초판 PL8 은 stamp property 가 특정 summary label 인지를
  //   **모양으로** 단언했다. 그런데 이 리포의
  ...
  it('PL8b: 검증기가 선언되지 않은 키와 알 수 없는 `env` 를 **실제로 거부한다**', () => {
    ...
    expect(validateItem({ ...valid, extraKey: 'evil/forged' }, schema, '$')).not.toEqual([])

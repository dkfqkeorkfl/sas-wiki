...
//
// ── 왜 이 파일이 따로 있는가 ─────────────────────────────────────────────────────────────────
// 이 케이스들은 **exit code · stdout · stderr** 를 무는 **프로세스 경계** 계약이라, 함수를 직접 부르는
//   in-process 파일에 들어갈 수 없다. 인자 계약을 무는 형제 파일(`wiki.summary-arg.test.mjs`)은
//   **소멸하는 계약**(`--summary` 필수화)을 담고 있어 거기에 새 계약을 얹으면 한 파일이 서로 배타적인
//   두 계약을 동시에 주장하게 된다. 그래서 신규 파일이고, 옛 파일의 처분은 **구현 착륙 이후**다
//   (이 커밋은 가산적이다 — 오늘 green 인 계약을 지우지 않는다).
//
// ── 관측 층 ──────────────────────────────────────────────────────────────────────────────────

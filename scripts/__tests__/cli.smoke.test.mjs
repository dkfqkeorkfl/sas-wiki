...
const SAVED_GIT_ENV = {}
// REPO_ROOT 를 기본 vault 로 쓰는 summary·feeds arm 의 git 소유권 방어만 남긴다. wiki 는 이제
//   `--file` 로 tmp vault 문서를 직접 읽으므로 실 저장소 프리빌드 산출물을 소비하지 않는다.
beforeAll(() => {
  for (const [k, v] of [
    ['GIT_CONFIG_COUNT', '1'],
...
describe('wiki.mjs CLI smoke', () => {
  // 옛 「없는 path → null」 명부 조회와 「`--vault` 누락 → 기본값」은 그 개념이 사라졌고,
  // 「`--summary` 누락 → exit 2」의 인자 오류 분담은 `wiki.file-arg.test.mjs` C-2가 이어받았다.
  it('wiki --file → 정확한 3키 JSON과 문서 본문', async () => {
    const file = path.join(vault, 'wiki', 'company', '삼성.md')
    const json = JSON.parse(await run(wikiMain, ['--file', file]))

    expect(Object.keys(json).sort()).toEqual(['md', 'meta', 'status'])
    expect(json.md).toContain('갱신.')
  })
})

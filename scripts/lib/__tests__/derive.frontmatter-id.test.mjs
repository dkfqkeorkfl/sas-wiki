...
//
// RED 사유: 현행 deriveForTest(derive.mjs:34-41)는 `id: dates.hash.slice(0, 12)` 로 **git 생성 해시**를
...
describe('R2 derive — id 는 frontmatter UUIDv7, created/updated 는 git 파생', () => {
  it('docs[0].id 가 frontmatter UUIDv7 이다(git 해시 slice 아님)', () => {
    const runner = makeRunner({ 'tech/HBM': aLog(GIT_HASH, 'wiki/tech/HBM.md') })

    const { docs } = deriveForTest([aParsedDoc('tech/HBM', { id: UUIDV7_A })], runner)
  ...
  it('disable 스텁도 frontmatter UUIDv7 id 를 그대로 싣는다', () => {
    const runner = makeRunner({ 'legacy/폐문서': aLog(GIT_HASH, 'wiki/legacy/폐문서.md') })
